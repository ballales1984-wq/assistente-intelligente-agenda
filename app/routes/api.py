...
def telegram_webhook():
    """
    Webhook endpoint per Telegram Bot
    Riceve aggiornamenti da Telegram e risponde automaticamente
    """
    from flask import current_app
    import asyncio
    
    try:
        # Ottieni dati webhook
        update_data = request.get_json()
        
        if not update_data:
            return jsonify({"error": "No data received"}), 400
        
        current_app.logger.info(f"📱 Telegram webhook ricevuto: {update_data.get('message', {}).get('text', 'N/A')[:50]}...")
        ...
        if not bot_service.is_available():
            current_app.logger.error("❌ Telegram Bot non disponibile (TELEGRAM_BOT_TOKEN mancante)")
            return jsonify({"error": "Bot not configured"}), 503
        
        # Processa webhook in modo asincrono
        try:
            # Crea event loop se non esiste
            try:
            ...
            # Esegui processing asincrono
            result = loop.run_until_complete(bot_service.process_webhook(update_data))
            
            if result.get("success"):
                current_app.logger.info(f"✅ Telegram webhook processato con successo")
                return jsonify({"ok": True}), 200
            else:
                current_app.logger.error(f"❌ Errore processing webhook: {result.get('error')}")
                return jsonify({"ok": False, "error": result.get("error")}), 500
                
        except Exception as e:
            current_app.logger.error(f"❌ Errore asincrono webhook: {e}")
            return jsonify({"ok": False, "error": str(e)}), 500
    
    except Exception as e:
        current_app.logger.error(f"❌ Errore generale webhook: {e}")
