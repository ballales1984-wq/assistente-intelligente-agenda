...
@cache.cached(timeout=60, query_string=True)  # 1 minuto cache, per lingua/categoria
def get_reflections():
    """
    ...
    query = ReflectionShare.query.filter_by(approved=True, language=language).filter(
    ...
    # Order by recent
    reflections = query.order_by(ReflectionShare.created_at.desc()).limit(limit).all()
...
def create_reflection():
    ...
    if len(text) < 20:
        return (
            jsonify(
                {"success": False, "error": "Reflection too short (min 20 characters)"}
            ),
            400,
        )
    ...
    if len(text) > 5000:
        return (
            jsonify(
                {"success": False, "error": "Reflection too long (max 5000 characters)"}
            ),
            400,
        )
...
def delete_reflection(reflection_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Reflection deleted"})
...
def add_reaction(reflection_id):
    ...
    if reaction_type not in valid_types:
        return (
            jsonify(
                {
                    "success": False,
                    "error": f"Invalid reaction type. Use: {valid_types}",
                }
            ),
            400,
        )
    ...
    existing = Reaction.query.filter_by(
        user_id=profilo.id, reflection_id=reflection_id
    ).first()
    ...
    if existing:
        ...
        # Create new reaction
        reaction = Reaction(
            user_id=profilo.id, reflection_id=reflection_id, reaction_type=reaction_type
        )
...
def get_comments(reflection_id):
    ...
    comments = (
        Comment.query.filter_by(
            reflection_id=reflection_id, parent_id=None, approved=True  # Solo top-level
        )
    ...
    for comment in comments:
        ...
        # Add replies
        replies = (
            Comment.query.filter_by(parent_id=comment.id, approved=True)
            .order_by(Comment.created_at.asc())
            .all()
        )
...
def create_comment(reflection_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "data": comment.to_dict(), "message": "Comment added"}
            ),
            201,
        )
...
def get_my_circles():
    ...
    return jsonify({"success": True, "data": [c.to_dict() for c in circles]})
...
def create_circle():
    ...
    try:
        ...
        member = CircleMember(circle_id=circle.id, user_id=profilo.id, role="admin")
...
def join_circle(invite_code):
    ...
    existing = CircleMember.query.filter_by(
        circle_id=circle.id, user_id=profilo.id
    ).first()

    if existing:
        return jsonify(
            {"success": True, "message": "Already a member!", "data": circle.to_dict()}
        )

    # Add member
    member = CircleMember(circle_id=circle.id, user_id=profilo.id, role="member")
...
def get_circle_members(circle_id):
    ...
    return jsonify(
        {"success": True, "data": [m.to_dict() for m in members], "total": len(members)}
    )
...
def get_challenges():
    ...
    challenges = (
        Challenge.query.filter(
            Challenge.end_date >= today, Challenge.language == language
        )
        .order_by(Challenge.participants_count.desc())
        .limit(20)
        .all()
    )

    return jsonify({"success": True, "data": [c.to_dict() for c in challenges]})
...
def join_challenge(challenge_id):
    ...
    existing = ChallengeParticipation.query.filter_by(
        challenge_id=challenge_id, user_id=profilo.id
    ).first()
...
def challenge_checkin(challenge_id):
    ...
    if (
        ...
        and (today - participation.last_check_in.date()).days == 1
    ):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": participation.to_dict(), "message": message}
        )
...
def community_stats():
    ...
    top_categories = (
        db.session.query(
            ReflectionShare.category, func.count(ReflectionShare.id).label("count")
        )
        .filter_by(approved=True)
        .group_by(ReflectionShare.category)
        .order_by(func.count(ReflectionShare.id).desc())
        .limit(5)
        .all()
    )
