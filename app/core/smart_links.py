...
class SmartLinksManager:
    ...
    def generate_search_response(self, query: str, is_news: bool = False, max_results: int = 5):
        """
        ...
        """
        try:
            logger.info(f"🔗 Generating smart link response for: '{query}' (news: {is_news})")
            ...
            # Costruisci risposta
            response = f"🔍 Ho trovato {len(results)} {search_type} per **'{query}'**:\n\n"
    ...
    def process_message(self, text: str):
        ...
        search_response = self.generate_search_response(
            query=intent['query'],
            is_news=intent['is_news'],
            max_results=5
        )
        
        if search_response['success']:
            logger.info(f"✅ Smart links generated: {len(search_response['results'])} results")
            return {
